...
export default {
  ...
  methods: {
    ...
    store() {
        let self = this;
        axios.post( this.$apiAdress + '/api/admin/notes?token=' + localStorage.getItem("api_token"),
          self.note
        )
  ...
  mounted: function(){
    let self = this;
    axios.get( this.$apiAdress + '/api/admin/notes/create?token=' + localStorage.getItem("api_token"))
    .then(function (response) {
