...
export default {
  ...
  methods: {
    ...
    deleteNote ( id ) {
      let self = this;
      let noteId = id;
      axios.post( this.$apiAdress + '/api/admin/notes/' + id + '?token=' + localStorage.getItem("api_token"), {
        _method: 'DELETE'
      })
    ...
    getNotes (){
      let self = this;
      axios.get( this.$apiAdress + '/api/admin/notes?token=' + localStorage.getItem("api_token") )
      .then(function (response) {
